...
function DashboardHome() {
  return (
    <div className="w-full h-full flex flex-col  gap-5">
      ...
      <div className=" grid md:grid-cols-2 gap-8 item-start">
        <DashboardHomeCartOne
          title={`Internship Applied Recently`}
          description={`Internship Applied Recently`}
        />
        <DashboardHomeCartOne
...
export function DashboardHomeCartOne({ title, description }) {
  return (
    <div className="bg-white py-3 rounded-md border">
      <p className="px-4 py-4 font-normal">{title}</p>
      <div className="border-b-2 w-full"></div>
      <p className="px-4 py-4 text-sm text-black text-opacity-75">
        {description}
      </p>
    </div>
  );
